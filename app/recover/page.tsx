...
export default function RecoverPage() {
  ...
  const [localStorageData, setLocalStorageData] = useState<AppData | null>(
    null
  );
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-6">
      ...
      <p className="text-muted-foreground">
        Use this page to inspect your local storage and Supabase data, and to
        recover data if it appears lost.
      </p>
      ...
      <div className="grid gap-6 md:grid-cols-2">
              ...
              <>
                <div>
                  <p>
                    ...
                    {localStorageData.invoices?.length || 0}
                  </p>
                  <p>
                    <strong>Expenses:</strong>{" "}
                    {localStorageData.expenses?.length || 0}
                  </p>
                  <p>
                    <strong>Income:</strong>{" "}
                    {localStorageData.income?.length || 0}
                  </p>
                  <p>
                    <strong>Fixed Expenses:</strong>{" "}
                    {localStorageData.fixedExpenses?.length || 0}
                  </p>
                </div>
                <Button
                  ...
                  disabled={
                    !!recoveryStatus &&
                    recoveryStatus !==
                      "Recovery failed. Check console for details."
                  }
                >
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <Button onClick={checkSupabaseData}>Re-check Supabase Data</Button>
            {supabaseDataInfo ? (
              supabaseDataInfo.error ? (
                <p className="text-red-600">
                  Error:{" "}
                  {supabaseDataInfo.message ||
                    supabaseDataInfo.details?.message}
                </p>
              ) : (
                <div>
                  <p>
                    <strong>Data Found:</strong>{" "}
                    {supabaseDataInfo.data_found ? "Yes" : "No"}
                  </p>
                  <p>
                    <strong>Sync Key:</strong> {supabaseDataInfo.sync_key}
                  </p>
                  <p>
                    <strong>Invoices:</strong> {supabaseDataInfo.invoice_count}
                  </p>
                  <p>
                    <strong>Expenses:</strong> {supabaseDataInfo.expense_count}
                  </p>
                  <p>
                    <strong>Income:</strong> {supabaseDataInfo.income_count}
                  </p>
                  <p>
                    <strong>Last Sync:</strong>{" "}
                    {supabaseDataInfo.last_sync
                      ? new Date(supabaseDataInfo.last_sync).toLocaleString()
                      : "N/A"}
                  </p>
                  {supabaseDataInfo.invoice_ids &&
                    supabaseDataInfo.invoice_ids.length > 0 && (
                      <div className="mt-4">
